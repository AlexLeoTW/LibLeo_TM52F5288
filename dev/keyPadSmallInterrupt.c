#include "keyPadSmallInterrupt.h"
...
unsigned char p1Ctrl[i] = {0xfe, 0xfd, 0xfb, 0xf7};
unsigned keyTable[][] = {
	{ 1, 2 , 3, KeyA }, 
	{ 4, 5 , 6, KeyB }, 
	{ 7, 8 , 9, KeyC }, 
...
void scanKey()
{
	...
	_nop_();
	columnStatus = P1;
	columnStatus = columnStatus & 0xf0;	// extract only columnStatus
	columnStatus = columnStatus ^ 0xf0;	// set only changed bit to 1
	columnStatus = columnStatus >> 4;	// move meaningful bit to right
